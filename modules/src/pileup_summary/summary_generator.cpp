...
int get_labels(char base) {
    if(base == 'A' or base == 'a') return 2;
    if(base == 'C' or base == 'c') return 3;
    if(base == 'T' or base == 't') return 4;
    if(base == 'G' or base == 'g') return 5;
    return 1;
}
...
//                base_qualities = read.base_qualities.begin() + read_index, read.base_qualities.begin() + (read_index + cigar.length);
                ...
                if(ref_position - 1 >= ref_start &&
                   ref_position - 1 <= ref_end) {
                    ...
                    longest_insert_count[ref_position-1] = std::max(longest_insert_count[ref_position-1], (long long) alt.length());
                }
...
void SummaryGenerator::generate_labels(vector <type_read> &truth_reads, long long region_start, long long region_end) {
    for(auto &read:truth_reads) {
        int read_index = 0;
        long long ref_position = read.pos;
        int cigar_index = 0;
        int base_quality = 0;
...
void SummaryGenerator::generate_summary(vector<type_read> &reads,
    ...
    if(train_mode) {
        ...
        generate_labels(truth_reads, start_pos, end_pos);
    }
        ...
        for (long long pos = start_pos; pos <= end_pos; pos++) {
            ...
            if (longest_insert_count[pos] > 0) {
                for (int ii = 0; ii < longest_insert_count[pos]; ii++) {
                    ...
                    if(insert_labels[make_pair(pos, ii)]) labels.push_back(get_labels(insert_labels[make_pair(pos, ii)]));
        ...
        if(longest_insert_count[i] > 0) {
            ...
            for(int ii=0;ii<longest_insert_count[i];ii++) {
                ...
                for(int b=0; b<5; b++) {
                    ...
                    for(int j=0; j<4; j++) {
                        double val = 0.0;
                        if(coverage[i] > 0) val = insert_summaries[make_pair(make_pair(i, ii), get_base_bin(base) + j)] / coverage[i];
