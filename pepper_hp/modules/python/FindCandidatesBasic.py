...
import re
from pepper_hp.modules.python.CandidateFinderBasic import find_candidates_basic
from pepper_hp.modules.python.VcfWriterBasic import VCFWriter
...
def candidate_finder_basic(input_dir, reference_file, sample_name, output_path, threads):
    ...
    vcf_file_hp1 = VCFWriter(reference_file, all_contigs, sample_name, output_path, "candidates_as_variants_hp1")
    vcf_file_hp2 = VCFWriter(reference_file, all_contigs, sample_name, output_path, "candidates_as_variants_hp2")
    ...
    for contig in sorted(all_contigs, key=natural_key):
        ...
        all_candidate_positions_hp1, candidate_positional_map_hp1 = \
            find_candidates_basic(input_dir,  reference_file, contig, all_chunk_keys, threads, 1)

        all_candidate_positions_hp2, candidate_positional_map_hp2 = \
    ...
    hdf5_file.close()
    vcf_file_hp1.close_file()
    vcf_file_hp2.close_file()

    haploid2diploid(output_path + "candidates_as_variants_hp1.vcf",
                    output_path + "candidates_as_variants_hp2.vcf",
                    reference_file,
                    output_path,
                    adjacent=False,
                    discard_phase=False)
...
def process_candidates_basic(input_dir, reference, sample_name, output_dir, threads):
    output_dir = UserInterfaceSupport.handle_output_directory(output_dir)
    candidate_finder_basic(input_dir,
                           reference,
                           sample_name,
                           output_dir,
                           threads)