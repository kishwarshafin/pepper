...
def filter_candidate(candidate_type, depth, read_support, read_support_h0, read_support_h1, read_support_h2, alt_prob_h1, alt_prob_h2, non_ref_prob):
    ...
    if candidate_type == 1:
        if 0.05 <= allele_frequency < 0.10:
            if non_ref_prob >= 0.8:
                return True
        elif 0.10 <= allele_frequency < 0.15:
            if non_ref_prob >= 0.6:
                return True
        elif non_ref_prob >= 0.1:
            return True

        if max(alt_prob_h1, alt_prob_h2) >= 0.5:
            return True
        if max(non_ref_prob, alt_prob_h1, alt_prob_h2) >= 0.8:
            return True
    # insert alleles
