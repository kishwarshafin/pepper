    ...
    def create_summary(self, truth_bam_handler_h1, truth_bam_handler_h2, train_mode, downsample_rate, bed_list):
        ...
        if train_mode:
            ...
            for reg_h1 in regions_h1:
                reg = AlignmentSummarizerHP.range_intersection([reg_h1] + regions_h2)
                if reg is not None:
            ...
            if bed_list is not None:
                ...
                for region in truth_regions:
                    if self.chromosome_name in bed_list.keys():
                        reg = AlignmentSummarizerHP.range_intersection_bed(region, bed_list[self.chromosome_name])
            ...
            for region_start, region_end, truth_read_h1, truth_read_h2 in truth_regions:
                ...
                regional_summary = PEPPER_VARIANT.RegionalSummaryGeneratorHP(region_start, region_end, ref_seq)
                ...
                #############################
                all_images_hp1.extend(regional_image_summary.chunked_image_matrix_hp1)
                all_images_hp2.extend(regional_image_summary.chunked_image_matrix_hp2)
                all_labels_hp1.extend(regional_image_summary.chunked_labels_hp1)
                all_labels_hp2.extend(regional_image_summary.chunked_labels_hp2)
            ...
            regional_summary_hp = PEPPER_VARIANT.RegionalSummaryGeneratorHP(self.region_start_position, self.region_end_position, ref_seq)

            regional_summary_hp.generate_max_insert_summary(all_reads)

            regional_image_summary = regional_summary_hp.generate_summary(all_reads,
                                                                          ImageSizeOptions.SEQ_OVERLAP,
                                                                          ImageSizeOptions.SEQ_LENGTH,
                                                                          ImageSizeOptions.IMAGE_HEIGHT,
                                                                          chunk_id_start,
        ...
        return all_images_hp1, all_images_hp2, all_labels_hp1, all_labels_hp2, all_positions, all_index, all_chunk_ids
